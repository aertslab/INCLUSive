  ...
  : MotifSamplerRun(pFasta, strand) 
{
  ...
  _pPriorMotifModel = new double*[_w];
  for (int i=0; i<_w; i++)
  {
    _pPriorMotifModel[i] = new double[4];
    for (int j=0; j<4; j++)
  ...
  PriorMotifSamplerRun::SetNewPriorMotifModel(PWM* pPriorMotif)
{
  ...
  if ( pPriorMotif->Length() != _w ){
    // delete old _pPriorMotifModel
    if ( _pPriorMotifModel != NULL )
    {
      for (int i=0; i<_w; i++)
        delete[]  _pPriorMotifModel[i];
      delete[] _pPriorMotifModel;
    }
    ...
    // reinitialize prior motif model
    _pPriorMotifModel = new double*[_w];
    for (int i=0; i<_w; i++)
      _pPriorMotifModel[i] = new double[4];
  ...
  PriorMotifSamplerRun::CoreSamplingStep(int iterations)
{
  ...
  for (int i = 0; i < iterations; i++)
  {
    ...
    while (mi != _pComputationMap->end())
    {
      ...
      pComp->UpdateCopyProbability(_strand);//3.1.5
